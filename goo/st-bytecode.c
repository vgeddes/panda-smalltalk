...
st_bytecode_new (void)
{
	...
	code->buffer = g_array_sized_new (false, true,
					  sizeof (char),
					  INITIAL_CAPACITY);
...
st_bytecode_append (GooBytecode *code, GooBytecode *another)
{
	...
	g_array_append_vals (code->buffer,
			     another->buffer->data,
			     another->buffer->len); 
...
st_bytecode_jump (GooBytecode *code, short offset)
{
	...
	if (-127 <= offset && offset <= 127) {
		...
		gen_code (code, BBB, long_jump,
			  HIGH_BYTE (offset), LOW_BYTE (offset));
	}
}
